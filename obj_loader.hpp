...
// Taken from https://github.com/Bly7/OBJ-Loader/blob/master/Source/OBJ_Loader.h
#ifndef OBJ_H 
#define OBJ_H
...
namespace objl
{
	...
	public:
		...
		bool LoadFile(std::string Path)
		{
			...
			for (long unsigned int i = 0; i < MeshMatNames.size(); i++)
			{
				std::string matname = MeshMatNames[i];

				// Find corresponding material name in loaded materials
				// when found copy material variables into mesh material
				for (long unsigned int j = 0; j < LoadedMaterials.size(); j++)
				{
			...
			const std::vector<Vertex>& iVerts)
		{
			...
			while (true)
			{
				...
				for (int i = 0; i < int(tVerts.size()); i++)
				{
					...
					Vertex pNext;
					if ((long unsigned int) i == tVerts.size() - 1)
					{
						pNext = tVerts[0];
					}
					else
